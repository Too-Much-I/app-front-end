...
`fileKey`는 서버가 만들어 내려주는 S3 객체 키지만 앱은 `submit`에 이를 되돌려주지 않는다. 서버는 `(examId, questionNumber, retryCount)`로 앞서 발급한 업로드 대상을 찾아야 한다. 웹 기준으로 `.webm`이 박혀 있을 수 있는데, **S3는 확장자를 신경 쓰지 않고 ffmpeg는 파일 내용으로 포맷을 판별**하므로 그대로 둬도 변환은 동작한다. 실제 문제가 되는 건 백엔드가 확장자로 분기하거나 거기서 MIME을 추론할 때뿐이다. 나중에 S3를 뒤질 때 헷갈리지 않도록 `.m4a`로 맞춰두는 걸 권한다.
...
```text
NotFoundException: Unable to find the native shared object associated with given JavaScript object
```
