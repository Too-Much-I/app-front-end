...
| 6 | **서버 상태** | **TanStack Query** | **의존성만 설치, 조회·폴링을 직접 구현** | **○ 큰 공백** |
...
현재 `use-*.ts(x)` 파일은 21개이고 내보낸 `use*` 함수 전체는 25개다. 이 전부가 서버 상태 훅은 아니지만,
조회·폴링 훅들이 `useState` + `useEffect` + `AbortController`로 각자 생명주기를 구현하는 것은 사실이다.
