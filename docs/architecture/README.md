...
어디가 이미 좋고, 어디가 다음 기능에서 아플지**를 그림 4장과 문서 4편으로 정리한 것이다.
...
1. **서버 상태 계층** — `@tanstack/react-query`를 2026-08-31(PR #41)에 쓰기 시작했지만
   `useQuery`는 아직 시험 이력 경로 두 곳뿐이다. 나머지 조회·폴링·취소·백오프는 훅마다
   각자 있고, 폴링만 세 벌이다.
...
| ~~7~~ | ~~`ExamHistoryScreen` 983줄 분해~~ — 완료(2026-09-01). `ExamHistoryTabView` 외 14개 파일(총 15개)로 분해, 최대 187줄 | | |
