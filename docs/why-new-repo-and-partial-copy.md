...
- `src/features/exam/use-answer-recorder.ts` — 브라우저 전용 녹음 구현은 RN에서 동작하지 않아 그대로 가져오지 않았다. 현재 파일은 `expo-audio` 기반의 Expo 네이티브 훅으로 새로 구현되어 답안 녹음 생명주기만 담당한다([answer-audio-recording-format.md](answer-audio-recording-format.md)).
- `src/features/exam/use-grading-progress.ts` — 처음에는 `"use client"` 지시어만 제거해 유지했지만, 채점 진행을 별도 `use-grading-status.ts`가 폴링하게 되면서 더 이상 사용되지 않아 삭제했다.
